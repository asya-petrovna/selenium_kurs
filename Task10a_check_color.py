...
def check_color(driver, price_selector, is_expected_color):
    ...
    red, green, blue = parse_color(price_color)
    assert is_expected_color(red, green, blue)


def parse_color(color_string):
    """Parse string of format rgba(NNN, NNN, NNN, N) into a tuple of integers (R, G, B)"""
    return tuple(re.findall(r'\d+', color_string)[:3])
